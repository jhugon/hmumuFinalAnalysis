...
meanG1 = root.RooRealVar("MeanG1","MeanG1", float(mass.replace("p",".")), float(mass.replace("p","."))-3, float(mass.replace("p","."))+3 )
...
mixGG = root.RooRealVar("mixGG","mixGG", 0.1140210709, 0.,1.0)

gaus1 = root.RooGaussian("gaus1","gaus1",mMuMu,meanG1,widthG1)
gaus2 = root.RooGaussian("gaus2","gaus2",mMuMu,meanG1,widthG2)
...
#  else:
  ...
  #if ( abs(float(mass.replace("p","."))-meanG1.getVal()) - abs(float(mass.replace("p","."))-meanG2.getVal()) < 0):
