...
#voitMmumu  = root.RooGenericPdf(channelName+"bak_voitMmumu","(@2)/(pow(@0-@1,2)+0.25*pow(@2,2))",root.RooArgList(dimuonMass,voitmZ,voitSig))
expParam = root.RooRealVar(channelName+"_expParam","expParam",-0.001,-0.1,0.1)
mm2Mmumu = root.RooGenericPdf(channelName+"bak_Mm2","exp(@0*@1)*pow(@0,-2)",root.RooArgList(dimuonMass,expParam))

expMmumu = root.RooGenericPdf(channelName+"bak_expMmumu","expMmumu","TMath::Exp(@0*@1)",root.RooArgList(dimuonMass,expParam))

expMm2Mmumu = root.RooProdPdf(channelName+"bak_expMm2Mmumu","expMm2Mmumu",root.RooArgList(expMmumu,mm2Mmumu))
expVoitMmumu = root.RooProdPdf(channelName+"bak_expVoitMmumu","expVoitMmumu",root.RooArgList(expMmumu,voitMmumu))

mixParam = root.RooRealVar(channelName+"_mixParam","mixParam",0.5,0,1)

#pdfMmumu = root.RooAddPdf("bak","bak",root.RooArgList(expVoitMmumu,expMm2Mmumu),root.RooArgList(mixParam))
...
pdfMmumu.fitTo(dimuonMassRooDataHist,root.RooFit.Range("lowsighigh"),root.RooFit.SumW2Error(False),PRINTLEVEL)
...
pdfMmumu.plotOn(plotMmumu,root.RooFit.Components(channelName+"bak_Mm2"),root.RooFit.LineStyle(2),root.RooFit.LineColor(root.kGreen+1),root.RooFit.Range("lowsighigh"))
